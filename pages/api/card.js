...
export default function Card(prop) {
  ...
  const {titolo, punteggio, altro, img, hue} = prop;
...
{/*<div className={`${styles.card} ${styles.playing} ${styles.blue}`}>
  
      <div className={styles.image} style={{background: 'url('+img+')', backgroundSize:'cover', backgroundRepeat:'no-repeat'}}></div>
      ...
      <div className={`${styles.info} text-light`}>
        <p className="t-elite mb-1">Terminato il: {altro}</p>
        <h2 className={`${styles.title}`}>{titolo}</h2>
        <div className={`${styles.point}`}>Punteggio: {punteggio}</div>
      </div>
    </div>*/}